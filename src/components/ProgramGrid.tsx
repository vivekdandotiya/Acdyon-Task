  ...
  return (
    <section id="programs" className="py-20 lg:py-28 bg-background relative overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-5 sm:px-6 lg:px-8">
        ...
        {/* 4 Cards Grid with Glass Effect & Circular Icon Badges */}
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-6">
                    {/* Circular Icon Badge */}
                    <div
                      className={`w-12 h-12 rounded-full flex items-center justify-center border transition-transform duration-200 group-hover:-translate-y-0.5 shadow-[0_6px_20px_rgba(0,0,0,0.18)] ${
                        isFeatured
                    ...
                    <div className="flex items-center space-x-2">
                      {isFeatured && (
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-white text-navy-950 border border-white shadow-subtle">
                  ...
                  <div className="space-y-2.5 mb-8">
                    {program.highlights.map((highlight, tIdx) => (
                      <div key={tIdx} className={`flex items-center text-xs sm:text-sm font-medium ${isFeatured ? 'text-slate-200' : 'text-slate-700'}`}>
                        <span className={`w-2 h-2 rounded-full mr-2.5 shrink-0 ${isFeatured ? 'bg-white' : 'bg-navy-950'}`} />
                        <span>{highlight}</span>
                      </div>
                    ))}
                  </div>
