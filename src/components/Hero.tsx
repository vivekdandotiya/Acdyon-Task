      ...
      <div className="max-w-[1280px] mx-auto px-5 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-10 items-center">
          ...
          <div className="lg:col-span-6 flex flex-col items-start text-left">
            {/* Editorial Eyebrow (150ms) - Small Editorial Text with Dot, NO Large Pill */}
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: heroTimeline.eyebrow.duration, delay: heroTimeline.eyebrow.delay, ease: easeOutCustom }}
              className="flex items-center space-x-2 text-xs sm:text-sm font-bold uppercase tracking-widest text-slate-700 mb-6 select-none"
            >
              {/* Single Subtle Entrance Pulse Dot */}
              <motion.span
                initial={{ scale: 0.5, opacity: 0.4 }}
                animate={{ scale: 1, opacity: 1 }}
                transition={{ duration: 0.6, delay: 0.2 }}
                className="w-2 h-2 rounded-full bg-acdyon-blue"
              />
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-[60px] font-bold text-navy-950 tracking-tight leading-[1.04] mb-7">
              ...
              >
                Find the{' '}
                <span className="relative inline-block text-acdyon-blue">
                  AI pathway
                  {/* Subtle 2px Accent Line Underneath "AI" */}
                  <motion.span
                    initial={{ scaleX: 0 }}
                    animate={{ scaleX: 1 }}
                    transition={{ duration: heroTimeline.blueHighlight.duration, delay: heroTimeline.blueHighlight.delay, ease: easeOutCustom }}
                    className="absolute -bottom-0.5 left-0 w-12 h-[2px] bg-acdyon-blue rounded-full origin-left opacity-75"
                  />
                </span>
              </motion.span>
            ...
            {/* CTA Group (820ms) - Only 2 Buttons, NO Extra Micro-Copy */}
            <motion.div
              initial={{ opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <motion.button
                ...
                whileHover={{ y: -2, boxShadow: '0 10px 25px rgba(37, 99, 235, 0.22)' }}
                whileTap={{ scale: 0.98 }}
                className="inline-flex items-center justify-center px-7 py-3.5 sm:py-4 rounded-xl text-base font-bold text-white bg-acdyon-blue hover:bg-acdyon-blueHover shadow-card transition-all duration-200 focus:outline-none group overflow-hidden"
              >
              ...
              <motion.button
                ...
                whileHover={{ y: -1, borderColor: '#cbd5e1' }}
                whileTap={{ scale: 0.98 }}
                className="inline-flex items-center justify-center px-6 py-3.5 sm:py-4 rounded-xl text-base font-semibold text-slate-700 bg-white hover:bg-slate-50 border border-slate-200 shadow-subtle transition-all duration-200 focus:outline-none group"
              >
                <Compass className="w-4 h-4 mr-2 text-slate-500 group-hover:translate-x-0.5 transition-transform duration-200" />
          ...
          >
            {/* Subtle Float Loop after settling (translateY 0 -> -3px -> 0 over 10s) */}
            <motion.div
              animate={{ y: [0, -3, 0] }}
