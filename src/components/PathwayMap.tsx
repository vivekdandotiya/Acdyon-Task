      ...
      <div className="max-w-[1280px] mx-auto px-5 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          ...
          <div className="lg:col-span-5 relative pl-6 sm:pl-8">
            {/* Animated Vertical Connection Line (scaleY 0 -> 1 over 1000ms) */}
            <div className="absolute top-4 bottom-4 left-[27px] sm:left-[35px] w-[2px] bg-slate-200 -z-0">
              <motion.div
                ...
                className="w-full h-full bg-gradient-to-b from-acdyon-blue via-acdyon-indigo to-acdyon-blue origin-top"
              />
            </div>
                ...
                return (
                  <motion.div
                    ...
                    className={`flex items-center p-3.5 sm:p-4 rounded-2xl border transition-all duration-300 cursor-pointer select-none ${
                      isActive
                        ? 'bg-slate-900 text-white border-slate-900 shadow-[0_8px_24px_rgba(15,23,42,0.15)] translate-x-2'
                        : 'bg-white text-slate-700 border-slate-200/90 hover:border-slate-300 hover:bg-slate-50'
                    }`}
                  >
                    {/* Node Badge Circle */}
                    <div
                      className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center mr-3.5 shrink-0 transition-transform duration-200 ${
                        isActive
                          ? 'bg-acdyon-blue text-white shadow-card'
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-2 mb-0.5">
                        <span className={`text-[10px] font-extrabold uppercase tracking-widest ${isActive ? 'text-acdyon-blueLight' : 'text-slate-400'}`}>
                          Node {node.step}
                        </span>
                      </div>
            ...
            {nodes.map((node) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-slate-50/90 rounded-3xl p-8 border border-slate-200/90 shadow-[0_12px_30px_rgba(0,0,0,0.06)] relative overflow-hidden"
                >
                  <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-200/80">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <span className="text-xs font-extrabold uppercase tracking-wider text-acdyon-indigo">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {node.details.map((detail, dIdx) => (
                        <div
                          key={dIdx}
                          className="bg-white p-3.5 rounded-xl border border-slate-200/80 text-xs font-semibold text-navy-950 flex items-center shadow-subtle"
                        >
                          <span className="w-2 h-2 rounded-full bg-acdyon-blue mr-2 shrink-0" />
                          <span>{detail}</span>
                        </div>
                      ))}
                    </div>
